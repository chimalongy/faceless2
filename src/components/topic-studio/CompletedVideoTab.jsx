...
export default function CompletedVideoTab({
  ...
  setShowShortsTitleOverlay,
  shortsOverlayText = "",
  setShortsOverlayText,
}) {
  const overlayDialogRef = useRef(null);
  const [overlayDraft, setOverlayDraft] = useState("");
  function openOverlayEditor() {
    setOverlayDraft(shortsOverlayText);
    overlayDialogRef.current?.showModal();
  }
  function toggleOverlay(checked) {
    if (checked) openOverlayEditor();
    else setShowShortsTitleOverlay(false);
  }
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <dialog ref={overlayDialogRef} aria-labelledby="overlay-text-heading"
        className="fixed inset-0 m-auto w-[calc(100%-2rem)] max-w-lg rounded-lg border border-line bg-paper-card p-6 text-ink shadow-xl backdrop:bg-black/60">
        <form onSubmit={(e) => {
          e.preventDefault();
          if (!overlayDraft.trim()) return;
          setShortsOverlayText(overlayDraft.trim());
          setShowShortsTitleOverlay(true);
          overlayDialogRef.current.close();
        }} className="space-y-4">
          <h3 id="overlay-text-heading" className="text-lg font-semibold">Overlay Text on Short</h3>
          <p className="text-sm text-ink-muted">Enter the text to display at the top throughout this Short. It will be added when you merge the video again.</p>
          <label htmlFor="short-overlay-text" className="block text-sm font-medium">Your overlay text</label>
          <textarea id="short-overlay-text" autoFocus required maxLength={120} rows={4}
            value={overlayDraft} onChange={(e) => setOverlayDraft(e.target.value)}
            placeholder="Enter your text here"
            className="w-full border border-line bg-paper p-3 text-ink rounded-sm" />
          <p className="text-xs text-ink-muted">{overlayDraft.length}/120 characters</p>
          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => overlayDialogRef.current.close()} className="px-4 py-2 border border-line">Cancel</button>
            <button type="submit" disabled={!overlayDraft.trim()} className="px-4 py-2 bg-signal text-white disabled:opacity-50">Save text</button>
          </div>
        </form>
      </dialog>
      {/* Top Banner Card */}
      <div className="p-4 sm:p-6 border border-line bg-paper-card space-y-4">
        ...
        {isShort && (
          <div className="flex items-center justify-between p-2.5 px-3 bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/80 dark:border-amber-800/40 text-xs rounded-xs">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="shorts-title-overlay-toggle"
                checked={showShortsTitleOverlay}
                onChange={(e) => toggleOverlay(e.target.checked)}
                className="w-4 h-4 text-signal rounded-xs border-line focus:ring-signal cursor-pointer"
              />
              <label htmlFor="shorts-title-overlay-toggle" className="cursor-pointer font-semibold text-ink flex items-center gap-1.5 select-none">
                <Sliders size={13} className="text-signal" />
                <span>Overlay Text on Short</span>
              </label>
            </div>
            <span className="text-[11px] text-ink-muted hidden sm:inline">
              Applies on the next merge. Merge again to add or remove your text.
            </span>
          </div>
        )}

        {isShort && (showShortsTitleOverlay || shortsOverlayText) && (
          <div className="flex items-center justify-between gap-3 text-sm">
            <p className="text-ink-muted break-words">{shortsOverlayText || "Enter your overlay text before merging."}</p>
            <button type="button" onClick={openOverlayEditor} className="shrink-0 font-semibold text-signal">Edit text</button>
          </div>
        )}
        {/* Status Notification */}
        {publishNotice && (
                ...
                <div className="text-center p-8 space-y-3">
                  ...
                  {isShort && (
                    <div className="flex items-center justify-center gap-2 pt-1 pb-1">
                      <input
                        type="checkbox"
                        id="shorts-title-overlay-empty-toggle"
                        checked={showShortsTitleOverlay}
                        onChange={(e) => toggleOverlay(e.target.checked)}
                        className="w-3.5 h-3.5 text-signal rounded-xs border-white/30 focus:ring-signal cursor-pointer"
                      />
                      <label htmlFor="shorts-title-overlay-empty-toggle" className="cursor-pointer text-xs font-medium text-white/90 select-none flex items-center gap-1.5">
