...
export default function TopicStudioPage() {
  ...
  const [shortsOverlayText, setShortsOverlayText] = useState("");
  ...
  // Load global defaults on initial mount
  useEffect(() => {
    fetch("/api/settings/endpoints")
      .then((res) => res.json())
      .then((data) => {
        if (data?.showShortsTitleOverlay !== undefined) {
          setShowShortsTitleOverlay(Boolean(data.showShortsTitleOverlay));
        }
      })
      .catch(() => {});
  }, []);
  ...
  async function handleGenerateSceneVideoModal(sceneNum, options = {}) {
    let parsed = [];
    try {
      parsed = JSON.parse(scenesJson);
    } catch {
  ...
  async function handleGenerateAllVideosModal(options = {}) {
    let parsed = [];
    try {
      parsed = JSON.parse(scenesJson);
    } catch {
  ...
  async function handleRenderMasterVideo(options = {}) {
    if (isShort && (options.showShortsTitleOverlay ?? showShortsTitleOverlay) && !shortsOverlayText.trim()) {
      toast.error("Click Edit text to enter your overlay text before merging.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/channels/${channelSlug}/topics/${topicSlug}/merge-scene-frames`, {
        ...
        body: JSON.stringify({
          ...
          isShort,
          showShortsTitleOverlay: isShort && (options.showShortsTitleOverlay !== undefined ? options.showShortsTitleOverlay : showShortsTitleOverlay),
          shortsOverlayText: shortsOverlayText.trim(),
        ...
        <>
          ...
          {activeTab === "completed_video" && (
            <CompletedVideoTab
              ...
              isShort={isShort}
              shortsOverlayText={shortsOverlayText}
              setShortsOverlayText={setShortsOverlayText}
              showShortsTitleOverlay={showShortsTitleOverlay}
              setShowShortsTitleOverlay={setShowShortsTitleOverlay}
