  ...
  useEffect(() => {
    async function loadEndpointsFromDb() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.nvidiaBaseUrl) setNvidiaBaseUrl(data.nvidiaBaseUrl);
    ...
    try {
      ...
      if (stored) {
        ...
        if (p.nvidiaBaseUrl !== undefined) setNvidiaBaseUrl(p.nvidiaBaseUrl);
              ...
              <div className="space-y-6">
                    ...
                    <div className="sm:col-span-4 space-y-1.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          disabled={!editingGeneral}
                          onClick={() => setDefaultLlmSource("nvidia")}
                          className={`flex-1 py-2 text-xs font-semibold rounded-xs border transition-all cursor-pointer ${
                            defaultLlmSource === "nvidia"
                              ? "bg-signal text-white border-signal shadow-xs"
                              : "bg-white border-line text-ink-muted hover:text-ink"
                          } ${!editingGeneral ? "opacity-80 cursor-not-allowed" : ""}`}
                        >
                    ...
                    <div className="sm:col-span-5 space-y-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          disabled={!editingGeneral}
                          onClick={() => setScriptGenSource("nvidia")}
                          className={`flex-1 py-2 text-xs font-semibold rounded-xs border transition-all cursor-pointer ${
                            scriptGenSource === "nvidia"
                              ? "bg-signal text-white border-signal shadow-xs"
                              : "bg-white border-line text-ink-muted hover:text-ink"
                          } ${!editingGeneral ? "opacity-80 cursor-not-allowed" : ""}`}
                        >
                    ...
                    <div className="sm:col-span-5 space-y-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          disabled={!editingGeneral}
                          onClick={() => setSceneGenSource("nvidia")}
                          className={`flex-1 py-2 text-xs font-semibold rounded-xs border transition-all cursor-pointer ${
                            sceneGenSource === "nvidia"
                              ? "bg-signal text-white border-signal shadow-xs"
                              : "bg-white border-line text-ink-muted hover:text-ink"
                          } ${!editingGeneral ? "opacity-80 cursor-not-allowed" : ""}`}
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                  ...
                  <div className="p-4 border border-line/70 bg-paper/50 rounded-xs space-y-2 md:col-span-2">
                    <label className="block text-xs font-semibold text-ink flex items-center justify-between" htmlFor="nvidia-base-url">
                      <span className="flex items-center gap-1.5">
                        <KeyRound size={13} className="text-signal" />
                    ...
                    />
                    <p className="text-[10px] text-ink-muted font-mono">
                      NVIDIA API Base URL for running NIM & open models via OpenAI SDK.
                    </p>
                  </div>
                  ...
                  {/* 5. Video Rendering & Shorts Defaults */}
                  <div className="p-4 border border-line/70 bg-paper/50 rounded-xs space-y-3 md:col-span-2">
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        <label className="block text-xs font-semibold text-ink flex items-center gap-1.5 cursor-pointer" htmlFor="shorts-title-overlay-toggle">
                          <Sliders size={13} className="text-signal" />
                          <span>Overlay Text on Shorts</span>
                        </label>
                        <p className="text-[11px] text-ink-muted">
                          Enable text overlays by default for Shorts. Enter the custom text in each topic’s Completed Video tab before merging.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer select-none">
                        <input
                          ...
                          onChange={(e) => setShowShortsTitleOverlay(e.target.checked)}
                          className="sr-only peer"
                        />
                        <div className={`w-9 h-5 bg-line peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all ${editingGeneral ? "peer-checked:bg-signal cursor-pointer" : "opacity-60 cursor-not-allowed peer-checked:bg-signal/60"}`}></div>
                      </label>
                    </div>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-line pb-2.5">
                        <div className="flex items-center gap-2 min-w-0 flex-1 flex-wrap">
                          ...
                          >
                            {sourceVal === "openrouter" ? "OpenRouter" : sourceVal === "nvidia" ? "NVIDIA" : "Gemini"}
                          </span>
                          {isEditing ? (
