...
export async function POST(req, context) {
  try {
    ...
    const resolvedShowShortsTitleOverlay = isShortTopic && Boolean(showShortsTitleOverlay);

    const overlayText = typeof shortsOverlayText === "string" ? shortsOverlayText.trim() : "";
    if (resolvedShowShortsTitleOverlay && (!overlayText || overlayText.length > 120)) {
      return NextResponse.json({ error: "Enter overlay text (1–120 characters) before merging." }, { status: 400 });
    }
    ...
    const handle = await tasks.trigger(taskId, {
      ...
      showShortsTitleOverlay: resolvedShowShortsTitleOverlay,
      topicTitle: topicTitle || topicSlug,
      shortsOverlayText: overlayText,
    });
