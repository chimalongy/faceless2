...
export function formatAssTime(seconds) {
  const total = Math.round(Math.max(0, Number(seconds) || 0) * 100);
  const hrs = Math.floor(total / 360000);
  const mins = Math.floor(total / 6000) % 60;
  const secs = Math.floor(total / 100) % 60;
  return `${hrs}:${String(mins).padStart(2, "0")}:${String(secs).padStart(2, "0")}.${String(total % 100).padStart(2, "0")}`;
}
...
export function extractWordsFromTranscription(transResult, sceneOffset = 0, fallbackText = "", sceneDuration = 0) {
  ...
  if (Array.isArray(transResult?.words) && transResult.words.length > 0) {
    for (const w of transResult.words) {
      ...
      const end = Number(w.end ?? w.end_time ?? (start - sceneOffset + 0.3)) + sceneOffset;
      words.push({
  ...
  if (Array.isArray(transResult?.segments) && transResult.segments.length > 0) {
    for (const seg of transResult.segments) {
      if (Array.isArray(seg.words) && seg.words.length > 0) {
        ...
        const segEnd = Number(seg.end ?? (segStart - sceneOffset + 2)) + sceneOffset;
  ...
  if (typeof transResult?.ass === "string" && transResult.ass.includes("Dialogue:")) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        const cleanText = match[3].replace(/\{[^}]+\}/g, "").replace(/\\+[Nnh]/g, " ").trim();
...
export function generateTikTokAss({
  ...
  // Build each scene independently. Never group words across scene boundaries.
  const bursts = [];
  let cumulativeOffset = 0;
  for (const scn of scenes) {
    const duration = Number(scn.duration) || 0;
    const raw = extractWordsFromTranscription(
      scn.transcription, 0, scn.audioText || scn.narration || "", duration
    );
    let previousEnd = 0;
    const words = [];
    for (const item of raw) {
      if (!Number.isFinite(item.start) || !Number.isFinite(item.end)) continue;
      const start = Math.max(0, previousEnd, item.start);
      const end = Math.min(duration, item.end);
      const word = item.word.replace(/\\+[Nnh]/g, " ").replace(/[{}\\]/g, "").trim();
      if (!word || Math.round(end * 100) <= Math.round(start * 100)) continue;
      words.push({ word, start: start + cumulativeOffset, end: end + cumulativeOffset });
      previousEnd = end;
    }
    bursts.push(...groupWordsIntoBursts(words, wordsPerBurst));
  ...
  for (const group of bursts) {
    ...
    for (let activeIdx = 0; activeIdx < group.length; activeIdx++) {
      ...
      const eventEnd = formatAssTime(nextWord ? Math.min(nextWord.start, activeWord.end) : activeWord.end);
  ...
  const assHeader = [
    ...
    `Style: TikTokCaption,DejaVu Sans,${fontSize},${chosenStyle.primaryColor},${chosenStyle.activeColor},${chosenStyle.outlineColor},${chosenStyle.backColor},-1,0,0,0,100,100,2,0,1,5.5,3,2,60,60,${marginV},1`,
    "",
    "[Events]",
