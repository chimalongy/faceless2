...
export const generateScenesTask = task({
  ...
  run: async (payload) => {
    ...
    const channelRows = await sql`
      SELECT id, name, slug, niche, sub_niche, description, mission, image_theme
      FROM channels
    ...
    const generalRows = await sql`
      SELECT 
        ...
        nvidia_base_url AS "nvidiaBaseUrl"
    ...
    const nvidiaBaseUrl = (genSettings.nvidiaBaseUrl || "https://integrate.api.nvidia.com/v1").trim();
    ...
    const channelMission = (channel.mission || channel.description || channel.name || "").trim();
    ...
    for (let i = 0; i < executionAccounts.length; i++) {
      ...
      const baseURL = resolveLlmBaseUrl(accountSource, {
        gemmaBaseUrl,
        openRouterBaseUrl,
        nvidiaBaseUrl,
      });
