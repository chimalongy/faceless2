...
export const extractZipImagesTask = task({
  ...
  run: async (payload) => {
    ...
    if (zipBase64) {
      ...
      throw new Error("ZIP data (zipBase64) must be provided to extract-zip-images task.");
    }
    ...
    // If a temporary key was passed, ensure it is deleted
    if (zipFileKey) {
      await deleteFromR2(zipFileKey).catch(() => {});
    }
