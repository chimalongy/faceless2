...
export const renderFrameVideoModalTask = task({
  ...
  run: async (payload) => {
    ...
    const burnSubtitles = payload.burnSubtitles ?? isShortTopic;
    if (burnSubtitles) {
      // Each request is transcribed independently. No cumulative scene offset.
      // Fail explicitly rather than silently publishing inaccurate/missing captions.
      for (const scene of scenesToRender) {
        const transcription = await transcribeAudio({
          audioUrl: scene.audioUrl,
          transcriptionUrl: payload.transcriptionUrl,
        });
        const words = transcription.words || transcription.segments?.flatMap(s => s.words || []);
        if (!Array.isArray(words) || !words.length || !(Number(transcription.duration) > 0)) {
          throw new Error(`Scene ${scene.scene_number}: word timestamps and audio duration are required. Deploy the updated whisper_modal.py.`);
        }
        scene.subtitles_ass = generateTikTokAss({
          scenes: [{ transcription: { words }, duration: Number(transcription.duration) }],
          styleKey: payload.subtitleStyle || "yellow_highlight",
          width: Number(renderWidth), height: Number(renderHeight),
          fontSize: Math.round(Number(renderHeight) * (isShortTopic ? 68 / 1920 : 52 / 1080)),
          marginV: Math.round(Number(renderHeight) * (isShortTopic ? 540 / 1920 : 120 / 1080)),
          wordsPerBurst: 3,
        });
        if (!scene.subtitles_ass.includes("Dialogue:")) {
          throw new Error(`Scene ${scene.scene_number}: no usable subtitle timestamps.`);
        }
      }
    }
