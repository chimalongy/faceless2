...
def normalize_subtitles_ass(content: str) -> str:
    ...
    def parse_time(value: str) -> int:
        ...
        if not match:
            raise MergeInputError(f"Invalid ASS subtitle timestamp: {value!r}")
        hours, minutes, seconds = map(int, match.groups()[:3])
        if minutes >= 60 or seconds >= 60:
            raise MergeInputError(f"Invalid ASS subtitle timestamp: {value!r}")
        return ((hours * 60 + minutes) * 60 + seconds) * 100 + int(
    ...
    for index, line in enumerate(lines):
        ...
        if stripped.lower().startswith("format:"):
            fields = [name.strip().lower() for name in stripped.split(":", 1)[1].split(",")]
            continue
        if not stripped.lower().startswith("dialogue:"):
            continue
        if not fields or fields[-1] != "text" or not {"start", "end"}.issubset(fields):
            raise MergeInputError("ASS Events require a Format with Start, End and Text last")
        values = stripped.split(":", 1)[1].lstrip().split(",", len(fields) - 1)
        if len(values) != len(fields):
            raise MergeInputError("Malformed ASS Dialogue event")
        start_index, end_index = fields.index("start"), fields.index("end")
        start, end = parse_time(values[start_index]), parse_time(values[end_index])
        # Only touch visible text; override tags contain meaningful backslashes.
        parts = re.split(r"(\{[^}]*\})", values[-1])
        for part_index in range(0, len(parts), 2):
            parts[part_index] = re.sub(r"\\+[Nnh]", " ", parts[part_index])
        values[-1] = "".join(parts)
        event_lines.add(index)
        if end > start:
    ...
    return "\n".join(
        ...
        for index, line in enumerate(lines)
        if index not in event_lines or index in replacements
    ) + "\n"
    ...
    try:
        ...
        if subtitles_ass and isinstance(subtitles_ass, str) and subtitles_ass.strip():
            ...
            subtitles_path.write_text(
                normalize_subtitles_ass(subtitles_ass), encoding="utf-8"
            )
