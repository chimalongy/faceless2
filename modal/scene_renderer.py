...
def get_audio_duration(audio_path: Path) -> float:
    try:
        ...
        pass
    raise RenderInputError("Unable to measure narration duration with ffprobe")
...
def prepare_scene(scene, fallback, job_dir, r2_client, bucket):
    ...
    return {
        ...
        "subtitlesAss": scene.get("subtitles_ass") or scene.get("subtitlesAss"),
        "outputPath": scene_dir / "final.mp4",
        "duration": get_audio_duration(audio_path),
    }


def allocate_image_frames(raw_timings, image_count, total_frames, fps):
    if image_count < 1 or total_frames < image_count * 2 or fps <= 0:
        raise RenderInputError("Scene is too short to render every image (two frames per image required)")
    duration = total_frames / fps
    durations = [duration / image_count] * image_count
    items = list(raw_timings.values()) if isinstance(raw_timings, dict) else raw_timings
    if isinstance(items, list) and len(items) == image_count:
        try:
            if any(isinstance(item, dict) and item.get("image_number") is not None for item in items):
                numbered = {int(item["image_number"]): item for item in items}
                if len(numbered) != image_count:
                    raise ValueError("Duplicate image number")
                items = [numbered[i + 1] for i in range(image_count)]
            proposed = [float(item["duration"]) if item.get("duration") is not None
                        else float(item["end_time"]) - float(item["start_time"]) for item in items]
            total = sum(proposed)
            minimum = min(0.5, duration / image_count)
            if (all(math.isfinite(d) and d > 0 and d >= minimum - 0.011 for d in proposed)
                    and math.isfinite(total)
                    and abs(total - duration) <= max(0.05, image_count * 0.011)):
                durations = [d * duration / total for d in proposed]
        except (TypeError, ValueError, KeyError, AttributeError, OverflowError):
            pass  # Malformed or stale plans use equal durations from actual audio.
    frames = []
    allocated = 0
    cumulative = 0.0
    for i, segment_duration in enumerate(durations):
        cumulative += segment_duration * fps
        remaining = image_count - i - 1
        target = total_frames if not remaining else min(
            total_frames - remaining * 2,
            max(allocated + 2, math.floor(cumulative + 0.5)),
        )
        frames.append(target - allocated)
        allocated = target
    return frames
...
def render_scene(scene, zoom_amount, pan_zoom, fps, width, height, threads):
    ...
    if not math.isfinite(duration) or duration <= 0:
        raise RenderInputError("Measured narration duration must be positive")
    ...
    if num_images <= 1:
        ...
        seg_frames_list = allocate_image_frames(image_timings, num_images, total_frames, fps)
        print(f"Scene {scene['sceneIndex']}: images={num_images}, total_frames={total_frames}, segment_frames={seg_frames_list}")
    ...
    if scene.get("subtitlesAss"):
        subtitles_path = scene["outputPath"].parent / "captions.ass"
        subtitles_path.write_text(scene["subtitlesAss"], encoding="utf-8")
        escaped = str(subtitles_path).replace(":", r"\:")
        video_encoding = [
            "-vf", f"setpts=PTS-STARTPTS,ass='{escaped}'",
