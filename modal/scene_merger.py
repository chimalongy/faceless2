...
def generate_title_card_png(
    ...
    """Generate a crisp, high-resolution aesthetic title card badge sticker for Shorts.

    Creates a rounded translucent dark badge with subtle drop-shadow and vibrant,
    high-contrast, auto-wrapped text.
    """
    import textwrap
    ...
    chars_per_line = max(16, int((card_width - padding_x * 2) / (font_size * 0.58)))
    wrapped_lines = textwrap.wrap(clean_title, width=chars_per_line)
    if not wrapped_lines:
        wrapped_lines = [clean_title]

    line_height = int(font_size * 1.28)
    text_block_height = len(wrapped_lines) * line_height
    card_height = text_block_height + padding_y * 2

    canvas_w = card_width + 30
    canvas_h = card_height + 30

    img = Image.new("RGBA", (canvas_w, canvas_h), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    rx0, ry0 = 10, 10
    rx1, ry1 = rx0 + card_width, ry0 + card_height
    corner_radius = 28
    ...
    # Sleek dark pill badge background (obsidian slate with glowing border)
    draw.rounded_rectangle(
        [rx0, ry0, rx1, ry1],
        radius=corner_radius,
        fill=(15, 23, 42, 228),
    ...
    # Top accent indicator pill
    accent_bar_w = 64
    accent_bar_h = 5
    accent_x0 = rx0 + (card_width - accent_bar_w) // 2
    accent_y0 = ry0 + 10
    draw.rounded_rectangle(
        [accent_x0, accent_y0, accent_x0 + accent_bar_w, accent_y0 + accent_bar_h],
        radius=3,
        fill=(245, 158, 11, 230),
    )

    # Draw centered wrapped text
    curr_y = ry0 + padding_y + 4
    for line in wrapped_lines:
        bbox = draw.textbbox((0, 0), line, font=font)
        line_w = bbox[2] - bbox[0]
        curr_x = rx0 + (card_width - line_w) // 2

        draw.text((curr_x + 2, curr_y + 2), line, font=font, fill=(0, 0, 0, 220))
...
def run_ffmpeg_merge(
    ...
    if is_short and title_card_path and title_card_path.exists():
        command.extend(["-i", str(title_card_path)])
        top_y = 200 if height >= 1920 else int(height * 0.10)
        filter_complex = (
            f"[0:v]{scale_filter}[base];"
            f"[base][1:v]overlay=(W-w)/2:{top_y}:format=auto[v]"
        )
        command.extend([
            "-filter_complex",
            filter_complex,
...
def merge_all_job(
    ...
    try:
        ...
        is_short = bool(payload.get("isShort")) or resolution in ("1080x1920", "720x1280")
        show_shorts_title_overlay = bool(payload.get("showShortsTitleOverlay", False))
        topic_title = str(payload.get("shortsOverlayText") or "").strip()
        if is_short and show_shorts_title_overlay and (not topic_title or len(topic_title) > 120):
            raise MergeInputError("Enter overlay text (1–120 characters) before merging.")

        title_card_path = None
        if is_short and show_shorts_title_overlay and topic_title:
            try:
                card_png = job_dir / "title_card.png"
                w, _ = output_dimensions(resolution)
                title_card_path = generate_title_card_png(topic_title, card_png, width=w)
            except Exception as e:
                raise RuntimeError(
                    f"Title badge was requested but could not be generated: {e}"
                ) from e
...
def api():
    ...
    class MergeRequest(BaseModel):
        ...
        showShortsTitleOverlay: bool = False
        topicTitle: str = ""
        shortsOverlayText: str = Field(default="", max_length=120)
